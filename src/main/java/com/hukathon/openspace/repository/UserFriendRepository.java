...
public interface UserFriendRepository  extends JpaRepository<UserFriend, Integer> {
    ...
    @Query("SELECT uf.user_id2 FROM UserFriend uf " +
            "WHERE (uf.user_id1.id = ?1 AND uf.friendStatus = 'FRIEND')")
    List<User> getFriendList1(Integer userId);

    @Query("SELECT uf.user_id1 FROM UserFriend uf " +
            "WHERE (uf.user_id2.id = ?1 AND uf.friendStatus = 'FRIEND')")
    List<User> getFriendList2(Integer userId);
