...
public interface UserFriendRepository  extends JpaRepository<UserFriend, Integer> {
    @Query("SELECT uf FROM UserFriend uf " +
            "WHERE (uf.user_id1 = ?1 AND uf.friendStatus = 'REQUEST_USER2') " +
            "OR (uf.user_id2 = ?1 AND uf.friendStatus = 'REQUEST_USER1')")
    List<UserFriend> getPendingRequest(Integer userId);

    @Query("SELECT uf FROM UserFriend uf " +
            "WHERE (uf.user_id1 = ?1 AND uf.friendStatus = 'REQUEST_USER2' AND uf.id = ?2) " +
            "OR (uf.user_id2 = ?1 AND uf.friendStatus = 'REQUEST_USER1' AND uf.id = ?2)")
    UserFriend getAReQuest(Integer userId, Integer userFriendId);

    @Query("SELECT uf FROM UserFriend uf " +
            "WHERE (uf.user_id1 = ?1 AND uf.friendStatus = 'FRIEND') " +
            "OR (uf.user_id2 = ?1 AND uf.friendStatus = 'FRIEND')")
